...
public class ScripsFileUploadController {
	...
	public String migrateFile(String fileUploadCode, BindingResult errors)
			throws Exception {
		String result = null;
		try {
			...
			if (fileUploadCode != null) {
				...
				if (fileHeaderObj != null) {
					...
					if(fileUploadCode.equals("SSS_SECURITIES_CODE_FILE_UPLOAD")||fileUploadCode.equals("SSS_SECURITIES_CODE_STATISTICS_FILE_UPLOAD")
							||fileUploadCode.equals("SSS_SECURITIES_POSITION_STATS_FILE_UPLOAD")||fileUploadCode.equals("SSS_SECURITIES_PRICE_FILE_UPLOAD")) {
						fileRecords =sssSecuritiesFileDataService.loadSecuritiesDataFile(fileUploadCode,fileHeaderObj);
					...
					if (fileRecords != null && fileRecords.size() > 0) {
						...
						if (fileRecords != null) {
							...
							case "sss_securities_position_stats":
								result = securitiesPositionStats.migrateSssSecuritiesPositionStats(fileHeaderObj, draftDBDetails,
										fileRecords);
								break;
			...
			fkChecksEnableDisable.fkStatus();
			movieSourceFile(result, fileUploadCode);
		}
		return result;
	}
	
	private void movieSourceFile(String result,String fileUploadCode) throws IOException
	{
		if (result !=null && env.getProperty("FTP.ENABLED") != null && env.getProperty("FTP.ENABLED").trim().equalsIgnoreCase("y")) {
...
//	private boolean preValidate(String fileUploadCode) {
//		if(env.getProperty("FTP.ENABLED") != null && env.getProperty("FTP.ENABLED").trim().equalsIgnoreCase("y")) {
//			if(sftpService.createChannelSftp()==null) {
//				logger.error("Unable to connect sftp server, Please check the sftp properties");
//			}else {
//				String fileNameWithPath = remoteFilePath + "/" + remoteFileName;
//			}
//		}
//		else if(env.getProperty("FTP.ENABLED") != null && env.getProperty("FTP.ENABLED").trim().equalsIgnoreCase("y"))
