...
CREATE TABLE `account_temp` (
  ...
  `compliance_calculation` varchar(1) DEFAULT '0' COMMENT 'Included for Compliance Calculation',
  `remarks` varchar(255) DEFAULT NULL,
  `minimum_balance` bigint DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uc_account` (`id`,`account_number`),
  KEY `fk_account_number_temp2` (`account_number`),
  CONSTRAINT `fk_account_number_temp1` FOREIGN KEY (`account_number`) REFERENCES `account_name_address` (`account_id`),
  CONSTRAINT `fk_account_number_temp2` FOREIGN KEY (`account_number`) REFERENCES `account_settlement_purpose` (`account_id`)
...
-- Dump completed on 2023-01-03 11:31:49
