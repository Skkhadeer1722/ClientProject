...
import scrips.datamigration.reports.dto.JpaSssFloatingRatesReportDto;
...
public interface SssFloatingRatesDAO extends JpaRepository <JpaSssFloatingRates, JpaSssFloatingRatesId> {
	...
	@Query("select new scrips.datamigration.reports.dto.JpaSssFloatingRatesReportDto( case when x.referenceRate is null then '' else x.referenceRate end AS x_reference_rate,\r\n"
			...
			+ "FROM JpaSssFloatingRatesSource z  left join\r\n"
			+ " JpaSssFloatingRatesTemp y on z.id=y.id left join\r\n"
			+ " JpaSssFloatingRates x on y.referenceRate=x.referenceRate AND y.valueDate =x.valueDate AND y.remarks is null  where :date is null or DATE(z.migratedDate)=:date  \r\n"
			+ "ORDER BY z.seq asc")
	List<JpaSssFloatingRatesReportDto> getReportData(@Param("date") Date date);
	@Query("select count(*) from JpaSssFloatingRatesTemp t inner join JpaSssFloatingRates l on t.referenceRate=l.referenceRate AND t.valueDate =l.valueDate and t.remarks is null where :date is null or DATE(l.createdDate)= :date")
