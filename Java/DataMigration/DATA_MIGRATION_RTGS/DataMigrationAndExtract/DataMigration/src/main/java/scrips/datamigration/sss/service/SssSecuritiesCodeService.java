...
public class SssSecuritiesCodeService {
	...
	private static String migrationFileName = null;

	@PostConstruct
	public void loadData() {
		migrationFileName = FileNameUtils.getBaseName(env.getProperty("STEPUP_FILE_NAME"));
	}
	...
	public String migrateSssSecuritiesCode(JpaFileUploadHeader fileHeaderObj, List<JpaFileUploadDetails> draftDBDetails,
		...
		try {
			...
			log.error("error while reading stepup data {}", e.getMessage());
			map = null;
		}
...
//					String arr1[] = (str).split("\\|");
			...
			try {
				directoryPath = new File(env.getProperty("remotefolderpath") + "/" + "file/MigrationFiles/");
				String contents[] = directoryPath.list();
				if (contents != null && contents.length > 0) {
					boolean isFileExists = false;
					for (String fileName : contents) {
						fileName = fileName.substring(0, fileName.indexOf("."));
						if (migrationFileName.equals(fileName)) {
							isFileExists = true;
							InputStream fileStream = null;
							fileStream = new BufferedInputStream(
									new FileInputStream(directoryPath + "/" + env.getProperty("STEPUP_FILE_NAME")));
					...
					if (isFileExists != true)
						log.error("Migration File is not found in Migration folder - {}",
								env.getProperty("STEPUP_FILE_NAME"));
				}
			} catch (Exception e) {
	...
	private List<JpaSssSecuritiesCodeSource> createAndSaveSecuritiesCodeSourceData(List<String> fileRecords,
		...
		try {
			...
			for (String line : fileRecords) {
				...
				String remarks = validationJpaSssSecuritiesCodeSource(jpaSssSecuritiesCodeSource);
...
//							.issueYield(Double.parseDouble(sourceRec.getIssueYield()))
				...
				e.printStackTrace();
				sourceRec.setRemarks("error while saving securities_code_temp table data");
				sssSecuritiesCodeSourceDAO.save(sourceRec);
			}
		}
				...
				try {
					...
					if (!remarks.isBlank() || !remarks.isEmpty()) {
						...
						try {
							...
							e.printStackTrace();
							noErrors = false;
							securitiesCodeTempObj.setRemarks("error while saving coupon_schedule table data");
							sssSecuritiesCodeTempDAO.save(securitiesCodeTempObj);
						}
					}
				} catch (Exception e) {
					securitiesCodeTempObj.setRemarks("error while saving securities_code live table data");
					e.printStackTrace();
					noErrors = false;
					sssSecuritiesCodeTempDAO.save(securitiesCodeTempObj);
				}
	...
	private String validateSecuritiesCodeLiveData(JpaSssSecuritiesCode securitiesCode) {
		...
		remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getRedemptionPrice()), "decimal",
				"Redemption Price"));
		remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getRedemptionPrice()), "decimal_9",
				"Redemption Price"));
		...
		if (securitiesCode.getOptionalRedemptionPrice() != null) {
			...
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getOptionalRedemptionPrice()),
					"decimal_9", "Optional Redemption Price"));
		}
		if (securitiesCode.getCouponRate() != null) {
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getCouponRate()), "decimal",
					"Coupon Rate"));
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getCouponRate()), "decimal_8",
					"Coupon Rate"));
		}
		if (securitiesCode.getAverageYield() != null) {
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getAverageYield()), "decimal",
					"Average Yield"));
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getAverageYield()), "decimal_9",
					"Average Yield"));
		}
		...
		if (securitiesCode.getIssuePrice() != null) {
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getIssuePrice()), "decimal",
					"Issue Price"));
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getIssuePrice()), "decimal_8",
					"Issue Price"));
		}
		if (securitiesCode.getIssueYield() != null) {
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getIssueYield()), "decimal",
					"Issue Yield"));
			remarks.append(regexValidation.regexValidator(String.valueOf(securitiesCode.getIssueYield()), "decimal_5",
					"Issue Yield"));
		}
...
//		                            .security(jpaSssSecuritiesCode)
							...
							log.info("ssb  added successfully.....");
						}
					}
